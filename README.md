...
`v0.2.0` 已推送到 GitHub 并发布。全仓 Shell 回归现为 41 个套件通过、0 个失败、1 个可选性能项跳过。浏览器已检查桌面登录页、语言切换和密码显隐；认证/MFA 端到端、响应式/无障碍及目标机服务与文件权限验证仍待完成。代码已发布但尚未部署；阶段详情和验证记录见 [`plan.md`](plan.md) 与 [`docs/M1_REPOSITORY_AUDIT.md`](docs/M1_REPOSITORY_AUDIT.md)。
