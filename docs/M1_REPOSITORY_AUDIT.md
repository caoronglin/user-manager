...
- Web 原生 inbox 使用单独 outbox/worker 投递固定模板的登录失败、Token 撤销、快照 freshness 变化事件；仅发送 allowlisted event type，不读取用户、IP、token 或通知详情。投递 worker 假定单个 `umweb` 进程共享该数据库。
- `web/frontend` 增加登录/MFA、只读信息页、WeCom 管理、投递记录、能力过滤、快照新鲜度提示、主题和中英文切换。页面代码不能代替浏览器实测。
- 前端 keyboard pass 增加 skip link/main landmark、可访问的菜单与表格操作、明确的空/错/加载状态及窄屏横向键盘滚动。完整 WCAG/读屏器审计未做。
...
| `cargo test --locked` | 通过，65 个测试 | 当前 backend 单元与集成测试；包含 WeCom native outbox/fixed payload/dry-run/retry classification、notifications、tokens、snapshot、auth；不覆盖真实 webhook 或目标机部署。 |
| `npm ci --offline` + `npm run build` | 通过 | 前端生产构建；npm audit 报告 0 个漏洞。构建有 Ant Design `use client` 提示和约 1.12 MB 主 JS bundle 提示。临时后端/browser 验证通过密码+TOTP、主要数据页、权限/错误状态与键盘交互；完整 WCAG/读屏器和目标机验收仍待完成。 |
...
此前审计记录的 shell 数值注入、子 shell FD 路径、Snapshot 错误处理、WeCom 缺失和 P5/P6 缺失已在当前工作树有对应实现。原生 WeCom HTTP 重试分支尚无 mock-server 测试，且多实例共享 SQLite 时没有跨进程 outbox lease；不要把单实例测试推断为多副本保证。

## 未完成项与发布边界

1. 目标 Ubuntu 主机与真实机器人验收仍待完成；开发阶段未发送外部消息。
