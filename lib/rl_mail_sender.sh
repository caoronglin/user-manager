...
rl_mail_send_raw() {
    ...
    [[ -n "$rl_recipient" ]] || {
        rl_mail_sender_msg msg_err "rl_mail_send: 收件人不能为空"
        return 1
    }
    [[ -n "$rl_content" ]] || {
        rl_mail_sender_msg msg_err "rl_mail_send: 邮件内容不能为空"
        return 1
    }
    case "$rl_backend" in
    auto)
        ...
        rl_mail_sender_msg msg_warn "未安装 msmtp/mailx，无法发送邮件"
        return 1
        ;;
    msmtp) printf '%s\n' "$rl_content" | timeout 30 msmtp "$rl_recipient" ;;
    mailx) printf '%s\n' "$rl_content" | timeout 30 mailx "$rl_recipient" ;;
...
rl_mail_send() {
    ...
    rl_from_name=$(rl_mail_config_get from_name 2>/dev/null)
    rl_from_addr=$(rl_mail_config_get from_address 2>/dev/null)
...
MAILEOF
    ...
    while ((rl_attempt < rl_max_retries)); do
        ...
        if ((rl_attempt < rl_max_retries)); then
            rl_wait_secs=$((rl_attempt * 2))
            sleep "$rl_wait_secs"
        fi
    done
    return 1
