...
_host_inventory_normalize_decimal_limit() {
    local value="${1:-}" max="$2" parsed
    [[ "$value" =~ ^[0-9]+$ && ${#value} -le ${#max} ]] || return 1
    parsed=$((10#$value))
    ((parsed >= 1 && parsed <= max)) || return 1
    printf '%d\n' "$parsed"
}

_host_inventory_config_limit() {
    local value="$1" default="$2" max="$3" normalized
    normalized="$(_host_inventory_normalize_decimal_limit "$value" "$max")" || normalized="$default"
    printf '%s\n' "$normalized"
}

HOST_INVENTORY_MAX_BYTES="$(_host_inventory_config_limit "${USER_MANAGER_HOSTS_MAX_BYTES:-65536}" 65536 1048576)"
HOST_INVENTORY_MAX_ROWS="$(_host_inventory_config_limit "${USER_MANAGER_HOSTS_MAX_ROWS:-256}" 256 4096)"
HOST_INVENTORY_TARGET_LIMIT="$(_host_inventory_config_limit "${USER_MANAGER_HOST_TARGET_LIMIT:-20}" 20 256)"
...
_host_inventory_add_builtin_local() {
    ...
    HOST_DISPLAY_NAME[local]='当前主机'
    HOST_PROVIDER[local]='local'
    HOST_ADDRESS[local]=''
    HOST_PORT[local]=''
    HOST_USER[local]=''
    HOST_GROUPS[local]='local'
    HOST_TAGS[local]='builtin'
...
_host_inventory_fd_real_path() {
    local fd="${1:-}" path
    [[ "$fd" =~ ^[0-9]+$ ]] || return 1
    path="$(readlink -f -- "/proc/$BASHPID/fd/$fd" 2>/dev/null)" || return 1
...
_host_inventory_validate_trusted_fd() {
    ...
    local path owner mode size file_type normalized_max_bytes
    normalized_max_bytes="$(_host_inventory_normalize_decimal_limit "$max_bytes" 1048576)" || {
        _host_inventory_error "文件大小上限非法"
        return 1
    }
    path="$(_host_inventory_fd_real_path "$fd")" || {
        _host_inventory_error "无法解析已打开文件"
        return 1
    }
    read -r owner mode size file_type < <(stat -Lc '%u %a %s %F' -- "/proc/$BASHPID/fd/$fd" 2>/dev/null) || {
        _host_inventory_error "无法读取文件元数据"
        return 1
    }
    [[ "$file_type" == "regular file" || "$file_type" == "普通文件" ]] || {
        _host_inventory_error "必须是普通文件"
        return 1
    }
    [[ "$owner" == "${EUID:-$(id -u)}" || "$owner" == "0" ]] || {
        _host_inventory_error "文件所有者不受信任"
        return 1
    }
    _host_inventory_mode_is_safe "$mode" || {
        _host_inventory_error "文件不得由组或其他用户写入"
        return 1
    }
    [[ "$size" =~ ^[0-9]+$ ]] && ((size <= normalized_max_bytes)) || {
        _host_inventory_error "文件超过允许大小"
        return 1
    }
    _host_inventory_acl_is_safe "$path" || {
        _host_inventory_error "扩展 ACL 不安全"
        return 1
    }
    _host_inventory_parent_chain_is_safe "$path" || {
        _host_inventory_error "父目录链不受信任"
        return 1
    }
}
...
_host_inventory_open_trusted_fd() {
    ...
    [[ -n "$path" && -n "$output_name" && -e "$path" && ! -L "$path" ]] || {
        _host_inventory_error "文件不存在或为符号链接"
        return 1
    }
    exec {opened_fd}<"$path" || {
        _host_inventory_error "无法打开文件"
        return 1
    }
...
_host_inventory_snapshot_trusted_file() {
    ...
    umask 077
    snapshot_dir="$(mktemp -d /tmp/user-manager-trusted-file.XXXXXX)" || {
        exec {fd}<&-
        return 1
    }
...
_host_inventory_commit() {
    ...
    for id in "${src_ids[@]}"; do
        HOST_DISPLAY_NAME["$id"]="${src_display[$id]}"
        HOST_PROVIDER["$id"]="${src_provider[$id]}"
        HOST_ADDRESS["$id"]="${src_address[$id]}"
        HOST_PORT["$id"]="${src_port[$id]}"
        HOST_USER["$id"]="${src_user[$id]}"
        HOST_GROUPS["$id"]="${src_groups[$id]}"
        HOST_TAGS["$id"]="${src_tags[$id]}"
        HOST_ENABLED["$id"]="${src_enabled[$id]}"
    done
}
...
host_inventory_load() {
    ...
    if (($# > 0)); then
        explicit=1
        file="$1"
    else file="${USER_MANAGER_HOSTS_FILE:-${DATA_DIR:-./data}/hosts.conf}"; fi
    if [[ ! -e "$file" ]]; then
        if ((explicit)); then
            _host_inventory_error "显式指定的文件不存在"
            return 1
        fi
        _host_inventory_add_builtin_local
        return 0
    ...
    while IFS= read -r line <&$fd || [[ -n "$line" ]]; do
        ...
        [[ ${#line} -le 1024 ]] || {
            _host_inventory_error "第 $line_number 行过长"
            exec {fd}<&-
            return 1
        }
        [[ "$line" != *$'\r'* && "$line" != *$'\t'* ]] || {
            _host_inventory_error "第 $line_number 行包含控制字符"
            exec {fd}<&-
            return 1
        }
        if ((header_seen == 0)); then
            [[ "$line" == "$HOST_INVENTORY_HEADER" ]] || {
                _host_inventory_error "表头不受支持"
                exec {fd}<&-
                return 1
            }
            header_seen=1
        ...
        ((data_rows <= HOST_INVENTORY_MAX_ROWS)) || {
            _host_inventory_error "主机条目超过上限"
            exec {fd}<&-
            return 1
        }
        IFS='|' read -r id display provider address port user groups tags enabled marker <<<"$line|__UM_END__"
        [[ "$marker" == "__UM_END__" ]] || {
            _host_inventory_error "第 $line_number 行字段数量错误"
            exec {fd}<&-
            return 1
        }
        [[ "$id" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$ ]] || {
            _host_inventory_error "第 $line_number 行 host_id 非法"
            exec {fd}<&-
            return 1
        }
        [[ -z "${providers[$id]:-}" ]] || {
            _host_inventory_error "host_id 重复"
            exec {fd}<&-
            return 1
        }
        [[ ${#display} -le 128 && ${#groups} -le 256 && ${#tags} -le 256 ]] || {
            _host_inventory_error "第 $line_number 行字段过长"
            exec {fd}<&-
            return 1
        }
        _host_inventory_valid_list "$groups" && _host_inventory_valid_list "$tags" || {
            _host_inventory_error "第 $line_number 行 groups/tags 非法"
            exec {fd}<&-
            return 1
        }
        enabled="$(_host_inventory_normalize_enabled "$enabled")" || {
            _host_inventory_error "第 $line_number 行 enabled 非法"
            exec {fd}<&-
            return 1
        }
        case "$provider" in
        local)
            [[ "$id" == "local" && -z "$address" && -z "$port" && -z "$user" ]] || {
                _host_inventory_error "本机条目不得携带远端连接字段"
                exec {fd}<&-
                return 1
            }
            ((local_count += 1))
            ((local_count == 1)) || {
                _host_inventory_error "本机条目重复"
                exec {fd}<&-
                return 1
            }
            ;;
        ssh)
            if [[ "$id" == "local" ]] || ! _host_inventory_valid_address "$address" ||
                ...
                _host_inventory_error "第 $line_number 行 SSH 连接字段非法"
                exec {fd}<&-
                return 1
            fi
            ;;
        *)
            _host_inventory_error "第 $line_number 行 provider 非法"
            exec {fd}<&-
            return 1
            ;;
        esac
        ...
        ports["$id"]="$port"
        users["$id"]="$user"
        groups_map["$id"]="$groups"
        tags_map["$id"]="$tags"
        enabled_map["$id"]="$enabled"
    done
    exec {fd}<&-
    ((header_seen == 1 && data_rows > 0)) || {
        _host_inventory_error "清单没有主机条目"
        return 1
    }
...
host_inventory_resolve() {
    ...
    all) wanted='' ;;
    group:*)
        wanted="${selector#group:}"
        [[ "$wanted" =~ ^[A-Za-z0-9._-]+$ ]] || return 1
        ;;
    host:*)
        wanted="${selector#host:}"
        [[ "$wanted" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$ ]] || return 1
        ;;
    *)
        wanted="$selector"
        [[ "$wanted" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$ ]] || return 1
    ...
    for id in "${HOST_INVENTORY_IDS[@]}"; do
        ...
        ((count += 1))
        ((count <= HOST_INVENTORY_TARGET_LIMIT)) || {
            _host_inventory_error "目标主机超过单次上限"
            return 1
        }
        printf '%s\n' "$id"
    done
