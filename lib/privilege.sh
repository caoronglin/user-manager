...
if [[ -z "${ACL_LEVEL_ROOT:-}" || -z "${ACL_LEVEL_ADMIN:-}" || -z "${ACL_LEVEL_USER:-}" || -z "${ACL_LEVEL_GUEST:-}" ]]; then
    ...
    # ============================================================
    # 密码传递加固：专用 chpasswd wrapper 路径
    # ============================================================

    # scripts/rl-chpasswd.sh 的绝对路径（密码经 stdin 传递，不落命令行/ps/审计日志）
    # 默认取部署路径 /usr/local/sbin/rl-chpasswd（与 sudoers 授权一致），不可执行时回退仓库内脚本；
    # 可用环境变量 RL_CHPASSWD_WRAPPER 覆盖（测试/定制部署用）。
    rl_priv_script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    rl_chpasswd_wrapper="${RL_CHPASSWD_WRAPPER:-/usr/local/sbin/rl-chpasswd}"
    [[ -x "$rl_chpasswd_wrapper" ]] || rl_chpasswd_wrapper="$(dirname "$rl_priv_script_dir")/scripts/rl-chpasswd.sh"
    ...
    readonly -A PRIV_CMD_WHITELIST=(
        ...
        ["rl-chpasswd"]="$ACL_LEVEL_ADMIN"
        ["rl-chpasswd.sh"]="$ACL_LEVEL_ADMIN"
    ...
    priv_chpasswd() {
        # 密码传递加固：从 stdin 读取 "user:password"（所有调用点均为
        # `echo "$username:$password" | priv_chpasswd`），解析后委托
        # scripts/rl-chpasswd.sh（仅密码经 stdin 传递，不落命令行/ps/审计日志）；
        # 保持返回语义：成功 0/失败非零。
        local rl_user_pass
        IFS= read -r rl_user_pass || {
            msg_err "priv_chpasswd: 无法从 stdin 读取 user:password"
            return 1
        }
        if [[ -z "$rl_user_pass" || "$rl_user_pass" != *:* ]]; then
            msg_err "priv_chpasswd: 需要 user:password 参数"
            return 1
        fi
        local rl_user="${rl_user_pass%%:*}"
        local rl_password="${rl_user_pass#*:}"
        if [[ -z "$rl_user" || -z "$rl_password" ]]; then
            msg_err "priv_chpasswd: user 或 password 不能为空"
            return 1
        fi
        printf '%s\n' "$rl_password" | priv_exec "$rl_chpasswd_wrapper" "$rl_user"
    }
