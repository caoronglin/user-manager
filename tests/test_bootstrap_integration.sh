...
missing_module_output="$(run_project_bootstrap 'set -uo pipefail; temp_root="$(mktemp -d)"; temp_lib="$temp_root/lib"; mkdir -p "$temp_lib"; cleanup() { rm -rf "$temp_root"; }; trap cleanup EXIT; modules=(common.sh config.sh access_control.sh privilege.sh quota_core.sh user_core.sh tui_core.sh); for module in "${modules[@]}"; do printf ":\n" > "$temp_lib/$module"; done; SCRIPT_DIR="$1"; LIB_DIR="$temp_lib"; source "$1/lib/bootstrap.sh"; um_load_profile full' || true)"
